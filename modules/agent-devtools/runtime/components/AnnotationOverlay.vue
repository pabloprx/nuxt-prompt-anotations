...
// Check if ID looks auto-generated by frameworks
function isAutoGeneratedId(id: string): boolean {
  // Vue scoped: v-0-46, data-v-xxx
  if (/^v-\d/.test(id) || /^data-v-/.test(id)) return true
  // Reka UI: reka-*, radix-*
  if (/^(reka|radix)-/.test(id)) return true
  // Headless UI, React: contains multiple hyphens with numbers
  if (/^[a-z]+-[a-z]+-[a-z0-9-]+$/i.test(id) && /\d/.test(id)) return true
  // UUID-like: 8-4-4-4-12 hex
  if (/^[a-f0-9]{8}-[a-f0-9]{4}-/i.test(id)) return true
  // Generic: ends with numbers or has :r: patterns (React)
  if (/:r\d+:/.test(id) || /-\d+$/.test(id)) return true
  return false
}
...
function buildSelector(el: HTMLElement): string {
  ...
  for (let i = 0; i < 5 && current && current !== document.body; i++) {
    ...
    // Prefer data-testid first (most stable)
    ...
    // Use ID only if it looks stable (not auto-generated)
    ...
    // Prefer classes (filter out framework-generated ones)
    if (current.className && typeof current.className === 'string') {
      const classes = current.className.trim().split(/\s+/).filter((c) => {
        // Skip Vue scoped, Tailwind state, and framework prefixes
        if (c.startsWith('v-') || c.startsWith('data-v-')) return false
        if (c.startsWith('reka-') || c.startsWith('radix-')) return false
        if (/^[a-z]+-\[\S+\]$/.test(c)) return false // Tailwind arbitrary: bg-[#fff]
        return true
      }).slice(0, 2)
